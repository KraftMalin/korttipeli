 ...
 *                                                                  *
 * Tiedosto: cardslot.cc                                            *
 * Kuvaus: Tämä tiedosto sisältää cardslotin public ja private      *
...
void CardSlot::addCard(Card *card) {
    if (topCard_ == nullptr){
        ...
        topCard_->stackCard(card, hiding_);
    }
    topCard_ = card;
    card->allowOpen();
}
...
// True jos allejäävät kortit pitää jäädä piiloon.
void CardSlot::setHiding() {
    hiding_ = true;
}
...
void CardSlot::dropEvent(QDropEvent *event) {
    ...
    // Lopetaan tapahtuma jos pinon päällimmäinen kortti on suljettu.
    if (topCard_ != nullptr && not topCard_->isOpen() ) {
        event->ignore();
        return;
    }

    // Tarkastetaan, onko sisältääkö pudotettu elementti tekstiä (korttidataa)
    // ja onko raahauksen aloituspaikka eri kuin pudotuspaikka.
    if (event->mimeData()->hasFormat("text/plain") && event->source() != this) {

        // Cardslotilla on funktio-osoitin checkFunction_, jonka osoittama
        // funktio tarkastaa, voiko pelisääntöjen mukaan pudotettavan/t kortin/t
        // pudottaa slotiin.
        // Ko. funktiolle annetaan parametrina kaksi stringiä, joista
        // ensimmäinen sisältää korttipaikassa päälimmäisenä olevan kortin
        // tiedot ja toinen lisättävistä korteista sen tiedot, joka olisi
        // tulossa päälimmäisenä olevan kortin päälle.

        // Pyydetään eventiltä tiedot siitä, mitä kortteja ollaan pudottamassa.
        QStringList newCardsData = event->mimeData()->text().split(";");

        // Jos newCardsData sisältää useamman kortin tiedot, ja yritetään
        // lisätä pino ylempiin cardslotteihin, hylätään eventti, koska sinne
        // ei saa kerralla lisätä kuin yhden kortin. Ylempien CardSlotien
        // attribuutti hiding_ on true, eli käytetään sitä hyväksi
        if (hiding_ && newCardsData.size() > 1) {
            event->ignore();
            return;
        }
        ...
        // Tarkastetaan checkFunction_ -osoittimen päässä olevaa funktiota
        // käyttäen, onko pudotus sallittu pelisääntöjen mukaan.
        if(checkFunction_(existingCardsData.back().toStdString(),
                          newCardsData.back().toStdString())) {
            // Jos pudotus halutaan hyväksyä, lisätään raahatut kortit tähän
            // slotiin.
...
void CardSlot::mousePressEvent(QMouseEvent *event) {
    ...
    if (!card->isOpen()){
        return;  // Jos kyseinen kortti on suljettu, ei klikatessa tehdä mitään.
    }
    ...
    // Muodostetaan uusi raahaus-event, johon laitetaan edellä tallennetut
    // tiedot.
    ...
    // Tämä ei ole olennaisinta sisältöä, mutta jos haluat tutkia hotspotin
    // ideaa, voit kokeilla korvata edellisen rivin jollakin seuraavista:
    // drag->setHotSpot();
    // drag->setHotSpot(event->pos());
    // drag->setHotSpot(event->pos() - card->pos());

    // Asetetaan harmautettu korttikuva väliaikaiseksi korttikuvaksi raahaamisen
    // ajaksi.
    ...
    if (drag->exec( Qt::MoveAction) == Qt::MoveAction) {
        ...
        if (card->parent() == this){
            ...
            // Jos korttipaikkaan jäi vielä kortteja, poistetaan poistuneet
            // kortit
